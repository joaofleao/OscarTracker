...
function MovieScreen({ navigation, route }: any) {
  ...
  const renderItem = ({ item }: ListRenderItemInfo<Nomination>) => (
    ...
    >
      <Text className="text-zinc-900 font-primaryBold text-base">
        {currentCategoriesMap.get(item.category)}
      </Text>
    </TouchableOpacity>
  )
    ...
    <ModelComponent>
      ...
      <View className="items-center ">
        ...
        <TouchableOpacity
          ...
          className={`py-4 px-4 rounded-2xl border-2 ${
      ...
      <View className="mt-4">
        <Text className="mx-4 mb-4 font-primaryBold text-white text-xl">
          Nominations
        </Text>
        <FlatList
          horizontal
          ...
          keyExtractor={(item) =>
            item.category + item.person + item.information
          }
